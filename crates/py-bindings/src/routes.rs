...
use openmobisim_core_routes::{
    DEFAULT_METHOD, Demand, LinkIndex, Registry, RouteKey, RouteSets, TripDemand,
};
...
/// owns routes `set_offsets[k]:set_offsets[k + 1]`, the method's routes best
/// first, then the routes a route update added, in the order added (`stamps`).
...
impl PyRouteSets {
    ...
    fn update(&self) -> String {
        self.inner.update().to_string()
    }

    /// A short identity of the network and the method with its options, in hex:
    /// equal exactly when both are, and the sets have been grown by the same
    /// update if they were.
    #[getter]
    ...
    /// The routes of the key at `key_index`, each as a uint32 array of link
    /// indices: the method's routes best first, then any a route update added.
    ///
    /// # Errors
    ///
...
///
/// A method that reads the demand (``"montecarlo"``, whose noise is biased towards the links the
/// demand will jam) is given these pairs as the demand: one traveller each, leaving at once.
///
...
pub fn route_sets_build(
    ...
    let sets = py.detach(move || {
        ...
        // A method that reads the demand (`"montecarlo"`'s bias) is given these pairs, one
        // traveller each, leaving at second 0.
        let generator = if generator.reads_demand() {
            let trips: Vec<TripDemand> =
                keys.iter().map(|&key| TripDemand { key, weight: 1, departure: 0 }).collect();
            generator
                .with_demand(&Demand { network: &net, turns: &turns, trips: &trips })
                .unwrap_or(generator)
        } else {
            generator
        };
